...
final class ManualRitualCasting {
    ...
    void attach(final BlockPos position, final List<RitualManager.RitualOption> updated) {
        ...
        update(updated);
        if (!selected.isEmpty()) ModClientNetwork.requestSelection(center, selected);
    }
    ...
    void selectSection(final String section) {
        ...
        selected = id;
        back();
        if (inRange() && !selected.isEmpty()) ModClientNetwork.requestSelection(center, selected);
    }
    ...
    void refresh() { if (inRange()) ModClientNetwork.requestRefresh(center); }
    ...
    void addWidgets(final ManualLayout layout, final Consumer<Button> add, final Runnable rebuild,
        ...
        final Button cast = Button.builder(Component.translatable(active ? "screen.warlockery.ritual.stop"
            : "screen.warlockery.ritual.begin"), button -> {
                if (active) ModClientNetwork.requestCancellation(center);
                else if (selected() != null && selected().ready()) ModClientNetwork.requestActivation(center, selected);
