...
public final class WerewolfVillagerInfectionRuntime {
    ...
    public static boolean markInfected(final Villager villager) {
        ...
        villager.getPersistentData().putBoolean(INFECTION_MARKER, true);
        return true;
    }

    public static boolean isInfected(final Entity entity) {
        return entity.getPersistentData().getBooleanOr(INFECTION_MARKER, false);
    }

    public static boolean isTransformedVillager(final Entity entity) {
        return entity.getPersistentData().getBooleanOr(TRANSFORMED_VILLAGER_MARKER, false);
    }
    ...
    static Optional<WerewolfEntity> convertInfectedVillager(
        ...
        werewolf.getPersistentData().putInt(ORIGINAL_XP, villager.getVillagerXp());
        werewolf.getPersistentData().put(ORIGINAL_VILLAGER_DATA, snapshot(villager));
        final long bloodDrainedUntil = VillageAssaultRuntime.bloodDrainedUntil(villager);
        if (bloodDrainedUntil > 0L) {
            werewolf.getPersistentData().putLong(
                VillageAssaultRuntime.BLOOD_DRAINED_UNTIL,
                bloodDrainedUntil
            );
        }
        villager.getVillagerData().profession().unwrapKey().ifPresent(key ->
            werewolf.getPersistentData().putString(ORIGINAL_PROFESSION, key.identifier().toString())
        );
        villager.discard();
    ...
    static Optional<Villager> restoreVillager(
        ...
        final Optional<CompoundTag> saved = werewolf.getPersistentData().getCompound(ORIGINAL_VILLAGER_DATA);
        ...
        villager.getPersistentData().putBoolean(INFECTION_MARKER, true);
        final long bloodDrainedUntil = werewolf.getPersistentData().getLongOr(
            VillageAssaultRuntime.BLOOD_DRAINED_UNTIL,
    ...
    private static void restoreLegacyState(
        ...
        villager.setVillagerXp(werewolf.getPersistentData().getIntOr(ORIGINAL_XP, 0));
        if (werewolf.getPersistentData().getBooleanOr(ORIGINAL_BABY, false)) {
    ...
    private static void restoreProfession(
        ...
        werewolf.getPersistentData().getString(ORIGINAL_PROFESSION)
            ...
            .ifPresent(identifier -> {
                ...
                villager.setVillagerData(villager.getVillagerData()
                    ...
                    .withLevel(Math.clamp(
                        werewolf.getPersistentData().getIntOr(ORIGINAL_LEVEL, 1),
                        1,
                        5
                    )));
            });
