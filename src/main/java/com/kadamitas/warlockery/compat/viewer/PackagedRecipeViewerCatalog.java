...
public final class PackagedRecipeViewerCatalog {
    ...
    private PackagedRecipeViewerCatalog() {
    }
    ...
    public static List<CustomBrewJeiRecipe> customBrews() { return CustomBrewCatalog.COMPONENTS; }
    ...
    private static BufferedReader reader(final String path) {
        final InputStream stream = Objects.requireNonNull(
            PackagedRecipeViewerCatalog.class.getResourceAsStream(path),
            "Missing packaged recipe resource " + path
        );
    ...
    private static final class CustomBrewCatalog {
        private static final List<CustomBrewJeiRecipe> COMPONENTS = load("custom_brews.txt", "custom_brew_component",
            com.kadamitas.warlockery.brew.custom.CustomBrewComponentDefinition.CODEC).stream()
            .map(entry -> new CustomBrewJeiRecipe(entry.identifier(), entry.value())).toList();
    }
}
