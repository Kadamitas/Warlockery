...
public final class LeonardBrewingRisk {
    ...
    public static boolean shouldBackfire(final RiskProfile profile, final float roll) {
        ...
        return profile.extenders() > 0 && roll < profile.chance();
    }

    public static boolean apply(final ServerLevel level, final BlockPos cauldron) {
        final RiskProfile profile = profile(nearbyExtenders(level, cauldron));
        ...
        level.getEntitiesOfClass(
            ...
            player.addEffect(new MobEffectInstance(effect, 300, profile.extenders() - 1));
            player.sendOverlayMessage(Component.translatable("message.warlockery.cauldron.leonard_backfire"));
        });
        level.sendParticles(
    ...
    public record RiskProfile(int extenders, int effectRange, float chance) {
        public RiskProfile {
            if (extenders < 0 || effectRange < 1 || chance < 0.0F || chance >= 1.0F) {
                throw new IllegalArgumentException("Invalid Leonard brewing risk profile");
            }
        }
    }
}
