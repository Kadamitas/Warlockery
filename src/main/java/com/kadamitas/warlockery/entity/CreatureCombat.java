...
public final class CreatureCombat {
    ...
    public static void handleDamage(final LivingDamageEvent.Pre event) {
        ...
        if (meleeWeapon != null
            ...
            event.setNewDamage(Math.max(event.getNewDamage(), event.getEntity().getMaxHealth() * 100.0F));
        }
    ...
    public static boolean isNullifyingHunterShot(final LivingDamageEvent.Pre event) {
    ...
    public static void capDeathDamage(final LivingDamageEvent.Pre event) {
        if (event.getEntity() instanceof ArcaneCreature creature
            && creature.creatureKind() == ArcaneCreature.CreatureKind.DEATH) {
            event.setNewDamage(DeathCombatRules.capIncoming(event.getNewDamage()));
        }
    }
    ...
    private static void applyPairedPatronProtection(
        ...
        event.setNewDamage(event.getNewDamage() * GoblinBossRules.pairedDamageMultiplier(distanceSquared));
    }

    private static void transferDamageToFamiliar(final LivingDamageEvent.Pre event) {
        ...
        final float transferred = event.getNewDamage()
