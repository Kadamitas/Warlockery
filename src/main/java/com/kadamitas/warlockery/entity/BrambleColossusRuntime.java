...
public final class BrambleColossusRuntime {
    ...
    private static void path(BrambleColossusEntity mob,Vec3 target,ServerLevel level){if(mob.colossusTransient().pathCooldown>0||mob.colossusTransient().routeBackoff>0||!token(level,q->q.paths<BrambleColossusRules.LEVEL_PATH_LIMIT&&q.expensive<BrambleColossusRules.LEVEL_EXPENSIVE_LIMIT&&++q.paths>0&&++q.expensive>0)){mob.colossusCounters().deferred++;return;}mob.colossusTransient().pathCooldown=20;var path=mob.getNavigation().createPath(target.x,target.y,target.z,0);if(path==null||!path.canReach()){if(BrambleColossusRules.thirdFailure(++mob.colossusTransient().routeFailures)){mob.recordPost(mob.blockPosition());cancel(mob);mob.colossusTransient().routeBackoff=BrambleColossusRules.routeBackoffSentinel();}return;}mob.colossusTransient().routeFailures=0;mob.getNavigation().moveTo(path,1);mob.colossusCounters().paths++;}
