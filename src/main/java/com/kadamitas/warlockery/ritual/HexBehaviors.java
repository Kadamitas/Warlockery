...
public final class HexBehaviors {
    private static final Map<String, Supplier<HexBehavior>> FACTORIES = Map.of(
        ...
        "heat_metal", () -> new PersistentHex(HexKind.HEAT_METAL),
    ...
    public static Optional<HexBehavior> find(final String target) {
        final Supplier<HexBehavior> factory = FACTORIES.get(target);
        return factory == null
            ? Optional.empty()
            : Optional.of(INSTANCES.computeIfAbsent(target, _ -> factory.get()));
    }

    public static HexBehavior require(final String target) {
        return find(target).orElseThrow(() -> new IllegalArgumentException("Unknown hex target: " + target));
    }
    ...
    public static boolean isPersistent(final String target) {
        return HexKind.find(target).isPresent();
    }
