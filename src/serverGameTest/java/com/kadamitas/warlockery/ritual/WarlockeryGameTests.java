...
public final class WarlockeryGameTests {
    ...
    public static void mendingDollTradesItsDurability(final GameTestHelper helper) {
        ...
        helper.runAfterDelay(21, () -> {
            doll.setDamageValue(doll.getMaxDamage() - 1);
            final ItemStack armorDoll = boundDoll(player, "armor_mending_doll");
            armorDoll.setDamageValue(armorDoll.getMaxDamage() - 1);
            final ItemStack armor = new ItemStack(Items.IRON_CHESTPLATE);
            armor.setDamageValue(5);
            player.setItemSlot(net.minecraft.world.entity.EquipmentSlot.CHEST, armor);
            helper.assertTrue(DollItem.tryMendBoundEquipment(doll, helper.getLevel(), player),
                "the final tool doll charge repairs and sends its activation without a consumed-stack cast");
            helper.assertTrue(DollItem.tryMendBoundEquipment(armorDoll, helper.getLevel(), player),
                "the final armor doll charge also repairs and sends its activation safely");
            helper.assertTrue(doll.isEmpty() && armorDoll.isEmpty(), "both exhausted dolls are consumed");
            helper.assertValueEqual(sword.getDamageValue(), 6, "final tool charge repairs exactly two durability");
            helper.assertValueEqual(armor.getDamageValue(), 3, "final armor charge repairs exactly two durability");
            helper.assertTrue(!DollItem.tryMendBoundEquipment(doll, helper.getLevel(), player)
                && !DollItem.tryMendBoundEquipment(armorDoll, helper.getLevel(), player),
                "empty dolls cannot perform additional repairs");
            helper.assertValueEqual(sword.getDamageValue(), 6, "exhausted tool doll leaves remaining damage alone");
            helper.assertValueEqual(armor.getDamageValue(), 3, "exhausted armor doll leaves remaining damage alone");
            helper.succeed();
        });
    }
    ...
    private static void removeTestAltarAtEnd(final GameTestHelper helper, final BlockPos relativeAltar) {
        helper.testInfo.addListener(new net.minecraft.gametest.framework.GameTestListener() {
            ...
            @Override public void testStructureLoaded(final net.minecraft.gametest.framework.GameTestInfo info) { }
            @Override public void testPassed(final net.minecraft.gametest.framework.GameTestInfo info,
                final net.minecraft.gametest.framework.GameTestRunner runner) { cleanup(); }
            @Override public void testFailed(final net.minecraft.gametest.framework.GameTestInfo info,
                final net.minecraft.gametest.framework.GameTestRunner runner) { cleanup(); }
            @Override public void testAddedForRerun(final net.minecraft.gametest.framework.GameTestInfo original,
                final net.minecraft.gametest.framework.GameTestInfo rerun,
                final net.minecraft.gametest.framework.GameTestRunner runner) { }
        });
    }
