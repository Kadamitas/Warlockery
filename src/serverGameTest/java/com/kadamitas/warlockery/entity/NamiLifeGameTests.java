...
public final class NamiLifeGameTests {
    ...
    public static void dailyRoutineReturnsHome(final GameTestHelper helper) {
        ...
        helper.assertTrue(nami.blockPosition().distSqr(home) > 4.0,
            "the return-home observation must begin outside the two-block home radius");
        final long observationStartedAt = helper.getTick();
        final long[] firstArrivalTick = {-1L};
        final BlockPos[] firstArrivalPosition = {null};
        // The contract is to return home during this window, not to remain home forever.
        final Runnable observeArrival = () -> {
            final long elapsed = helper.getTick() - observationStartedAt;
            if (firstArrivalTick[0] < 0L && elapsed <= 120L && nami.blockPosition().distSqr(home) <= 4.0) {
                firstArrivalTick[0] = elapsed;
                firstArrivalPosition[0] = nami.blockPosition().immutable();
                LOGGER.info("Nami return-home first arrival: tick={}, position={}, home={}",
                    elapsed, firstArrivalPosition[0], home);
            }
        };
        helper.onEachTick(observeArrival);
        helper.runAfterDelay(120, () -> {
            observeArrival.run();
            LOGGER.info("Nami return-home final observation: firstArrivalTick={}, firstArrivalPosition={}, "
                    + "position={}, home={}, state={}, counters={}",
                firstArrivalTick[0], firstArrivalPosition[0], nami.blockPosition(), home,
                nami.lifeState(), nami.lifeCounters());
            ...
            helper.assertTrue(firstArrivalTick[0] >= 0L,
                "nighttime shelter must return Nami within two blocks of home during 120 ticks; firstArrivalTick="
                    + firstArrivalTick[0] + ", firstArrivalPosition=" + firstArrivalPosition[0]
                    + ", finalPosition=" + nami.blockPosition()
