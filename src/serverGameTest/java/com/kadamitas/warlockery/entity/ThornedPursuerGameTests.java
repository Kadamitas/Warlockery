...
public final class ThornedPursuerGameTests {
    ...
    public static void thornedPursuerBaysBeforeItCommitsToACourse(GameTestHelper helper) {
        ...
        try {
            ...
            helper.assertValueEqual(WarlockeryEntityData.get(pursuer).getStringOr(
            ...
            helper.runAfterDelay(180, () -> fixture.step(() -> {
                ...
                helper.runAfterDelay(1, () -> fixture.step(() -> {
                    ...
                    helper.runAfterDelay(40, () -> fixture.step(() -> {
                        ...
                        // Natural course stamps already have at least one loaded tick of age.
                        // Age them out before measuring the new stamp's exact expiry boundary.
                        for (int loadedTick = 0; loadedTick < 199; loadedTick++)
                            ThornedPursuerRuntime.ageTrailLoadedTick(pursuer);
                        final long trailExpiriesAt199 = pursuer.pursuerCounters().trailExpiries;
                        ThornedPursuerRuntime.ageTrailLoadedTick(pursuer);
                        helper.assertValueEqual(pursuer.pursuerCounters().trailExpiries - trailExpiriesAt199, 1L,
                            "the seeded stamp survives 199 loaded ticks and expires on tick 200");
                        ThornedPursuerRuntime.ageTrailLoadedTick(pursuer);
                        helper.assertValueEqual(pursuer.pursuerCounters().trailExpiries - trailExpiriesAt199, 1L,
                            "the expired stamp cannot expire again on loaded tick 201");
    ...
    public static void thornedPursuerEscortIsOwnedCappedAndReleased(GameTestHelper helper) {
        runIsolated(helper, (fixture, pursuer) -> {
            ...
            for (java.util.UUID id : pursuer.pursuerRuntime().escortIds()) {
                ...
                helper.assertValueEqual(WarlockeryEntityData.get(wolf).getStringOr("WarlockeryHuntEscort", ""),
    ...
    private static void beginRealThornedPursuerRite(GameTestHelper helper, Fixture fixture) {
        ...
        helper.runAfterDelay(165L, () -> fixture.step(() -> {
            ...
            // Summons may land beyond the 15-block construction cell at the outer ritual radius.
            final AABB resultBounds = new AABB(center).inflate(definition.radius() + 1.0D);
            final double maximumSpawnDistance = 1.5D + Math.max(1, definition.radius() - 1);
            final var spawnCenter = net.minecraft.world.phys.Vec3.atBottomCenterOf(center).add(0.0D, 1.0D, 0.0D);
            for (var direction : net.minecraft.core.Direction.Plane.HORIZONTAL) {
                helper.assertTrue(resultBounds.contains(spawnCenter.add(
                        direction.getStepX() * Math.nextDown(maximumSpawnDistance), 0.0D,
                        direction.getStepZ() * Math.nextDown(maximumSpawnDistance))),
                    "the result query covers every permitted cardinal summon position");
            }
    ...
    private static void exerciseFixtureSixMatrix(GameTestHelper helper, Fixture fixture,
        ...
        try {
            for (Difficulty difficulty : Difficulty.values()) {
                ...
                WarlockeryEntityData.get(cohort).putBoolean("IsBaby", true);
                WarlockeryEntityData.get(cohort).putBoolean("CanBreakDoors", true);
                WarlockeryEntityData.get(cohort).putInt("ConversionTime", 200);
        ...
        ThornedPursuerEntity findHornPursuer(ServerPlayer owner) {
            ...
            helper.getLevel().getEntities(EntityTypeTest.forClass(ThornedPursuerEntity.class),
                new AABB(expected).inflate(1.0D), entity -> owner.getStringUUID().equals(
                    WarlockeryEntityData.get(entity).getStringOr("WarlockerySummoningOwner", "")), found, 4);
