...
Warlockery 1.5.1 targets Minecraft 26.2 and Java 25. The Forge build requires Forge 65.1.0 or newer; matching cross-loader builds target NeoForge 26.2.0.87 and Fabric Loader 0.19.5 with Fabric API 0.158.0+26.2. The normal NeoForge minimum remains 26.2.0.64. Codecs serialize data. They do not replace the shared item dictionary. Cross-mod substitution uses canonical `c:` tags, vanilla behavior tags, data-driven recipes, and Forge capabilities.

The `1.5.0-LlaGuiT0-26.2.0.45` supporter build is NeoForge-only for LlaGuiT0's modpack. Its NeoForge dependency is intentionally exact: `[26.2.0.45-beta,26.2.0.46-beta)`. It does not imply Forge or Fabric artifacts.
...
| Bolts and crossbows | `warlockery:crossbow_bolts` (crossbow-only ammunition) and Minecraft's vanilla crossbow |
...
Complete manuals are published in `warlockery:guide_books` and `minecraft:bookshelf_books`. The torn page stays in the broader `warlockery:manuals` tag but is intentionally excluded from the complete guide list. Forge 65 has no canonical `c:books` contract, so Warlockery does not invent one.
...
Silver, holy, stake, splitting, and anti-magic bolts are `ArrowItem` instances in `warlockery:crossbow_bolts`; they are deliberately absent from `minecraft:arrows` so bows never draw them, and the vanilla crossbow finds them through the mod's projectile-selection hook. Throwing Rocks extend vanilla `SnowballItem`. Fixed brews extend `SplashPotionItem`.

The fixed Brew of Combustion supplies 2,400 burn ticks through `Item.getBurnTime(ItemStack, RecipeType)`, which is Forge 65's item fuel hook. The inactive `brew.fuel` registry placeholder is not advertised as fuel.
...
Forge 65 has no universal mana capability. Its built-in capability set provides energy, fluid handlers, fluid container handlers, and item handlers. Warlockery altar power is not Forge Energy and is not labeled as another mod's mana. Optional mana support requires a soft adapter for each external mana API.
