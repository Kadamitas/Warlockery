# Warlockery Features

Warlockery combines data-driven circle rites, ritual machines, custom brewing, supernatural progression, sympathetic magic, magical creatures, village integration, and modern diagnostic interfaces. Floating displays identify missing requirements and show a green check when an interaction is ready.

## Using ritual circles

Draw the required chalk circles, provide a powered altar, and place the ingredients near the center. Use the center to open the ritual browser. Select a ritual to see each required glyph, ingredient, nearby entity sacrifice, structure part, and environmental condition. Activation is available when every line is complete and the green check is visible.

Golden Chalk normally places the single heart that opens a ritual. Crouch while placing it to draw golden ring marks for rites such as handfasting without creating extra ritual centers.

A hollow 3 by 3 ring of eight Ritual Chalk marks acts as a Listening Ring. Leave an Attuned Stone or Subdued Spirit in its empty center for three seconds to reveal the direction and distance to the nearest village in the Overworld or Nether fortress in the Nether.

## Ritual catalog

- **Anguish of the Dead**: Scours nearby undead with magic and leaves them weak and slow.
- **Portable Rite of Banishing**: Banishes nearby hostile supernatural creatures from a talisman circle.
- **Rite of Banishing**: Banishes hostile supernatural creatures from the ritual area.
- **Rite of Greater Protection**: Raises a wide magical ward for a coven and its allies.
- **Portable Rite of Protection**: Provides one minute of powerful warding.
- **Rite of Protection**: Raises a short-lived magical ward around the circle.
- **Portable Rite of Binding: Circle**: Refreshes a circle talisman for use away from a permanent altar.
- **Rite of Binding: Circle Talisman**: Pulls the prepared circle into a talisman for portable rites.
- **Rite of Binding: Familiar**: Strengthens and protects a familiar standing in the circle.
- **Rite of Binding: Fetish**: Consumes distinct spectral beings and writes the resulting protective mode into a bound Scarecrow.
- **Rite of Binding: Spectral Creatures**: Condenses spectral creatures into a refreshed Spectral Stone.
- **Rite of Binding: Statue**: Copies a sympathetic player imprint into a Hobgoblin Patron statue so offerings bless that bound player.
- **Rite of Binding: Blooded Waystone**: Creates a waystone bound through a creature sympathetic imprint.
- **Rite of Binding: Death**: Consumes five Banshees, five Poltergeists, and five Spectres on a full-moon night to summon Death.
- **Portable Rite of Binding: Waystone**: Binds a waystone at a talisman-powered circle.
- **Rite of Binding: Waystone**: Binds a waystone to the circle location.
- **Hex of Blight**: Destroys crops and flowers and leaves the land barren.
- **Hex of Blindness**: Blinds creatures caught inside the circle.
- **Rite of Beastial Call**: Calls a pack of wolves from the surrounding wilds.
- **Rite of Calling: Familiar**: Calls a familiar back to the warlock.
- **Rite of Calling: Circle Mage**: Calls a Circle Mage who can be recruited with a bound familiar and coven offering.
- **Rite of Calling: Cat Familiar**: Calls a Familiar Cat that can be bound as a persistent magical companion.
- **Rite of Charging**: Charges an attuned stone with concentrated altar power.
- **Rite of Shifting Seasons**: Calms severe weather and encourages a gentler local climate.
- **Rite of Broiling**: Cooks dropped raw food in the ritual area.
- **Portable Rite of Binding: Copy Waystone**: Copies a bound location at a talisman-powered circle.
- **Rite of Binding: Copy Waystone**: Copies a bound location onto a second waystone.
- **Hex of the Corrupted Doll**: Breaks protective enchantments around the imprint-bound being.
- **Rite of Hexbreaking: Insanity**: Cleanses insanity from the imprint-bound being.
- **Rite of Hexbreaking: Misfortune**: Cleanses misfortune from the imprint-bound being.
- **Rite of Hexbreaking: Nightmare**: Ends the waking nightmare of the imprint-bound being.
- **Rite of Hexbreaking: Overheating**: Cools and cleanses the imprint-bound being.
- **Rite of Hexbreaking: Sinking**: Lifts the sinking hex from the imprint-bound being.
- **Rite of Hexbreaking: Vampirism**: Cleanses vampirism from the imprint-bound vampire.
- **Rite of Hexbreaking: Lycanthropy**: Cleanses lycanthropy from the imprint-bound werewolf.
- **Deathly Veil**: Conceals beings inside the circle from ordinary sight.
- **Drain Growth**: Returns nearby crops to seedlings and transfers their growth into the caster's health and hunger.
- **Hex of Insanity**: Afflicts the imprint-bound being with visions and darkness.
- **Hex of Misfortune**: Hexes the imprint-bound being with persistent bad luck.
- **Hex of Waking Nightmare**: Haunts the imprint-bound being with darkness and hunger.
- **Hex of Overheating**: Causes the imprint-bound being to burn with infernal fever.
- **Heat Metal Hex**: Makes carried iron, gold, chain, and silver burn the imprint-bound victim until death or a bound Hex Guard Doll spends durability to break it.
- **Hex of Sinking**: Burdens the imprint-bound being with crushing supernatural weight.
- **Hex of the Wolf**: Bestows lycanthropy on the imprint-bound mortal under a full moon with a full coven.
- **Portable Rite of Total Eclipse**: Holds the server clock at night for a bounded duration from a circle talisman.
- **Rite of Total Eclipse**: Holds night for its duration, blankets nearby creatures in darkness, then restores the prior clock plus elapsed time.
- **Portable Rite of Fertility**: Accelerates nearby growth from a circle talisman.
- **Rite of Fertility**: Accelerates nearby crops, saplings, and flowers.
- **Rite of the Forest**: Grows surrounding saplings and spreads mature woodland.
- **Fortification of the Corpse**: Grants nearby undead temporary health, resistance, and strength.
- **Rite of Glyphic Transformation: Infernal**: Transforms the outer glyphs into infernal chalk.
- **Rite of Glyphic Transformation: Ritual**: Transforms the outer glyphs into ritual chalk.
- **Rite of Glyphic Transformation: the Veil**: Transforms the outer glyphs into Veil Chalk.
- **Hex of Hell on Earth**: Unleashes a horde of the dead in the Overworld at night.
- **Graveyard Mist**: Fills the circle with ash that restores and conceals undead while hindering the living.
- **Rite of Icy Expansion**: Builds a hollow sphere of packed ice around the circle.
- **Rite of Imprisonment**: Cripples movement inside the circle so captured foes cannot escape.
- **Rite of Infusion: Brew of the Grave**: Combines tagged Congealed Spirit and Graveyard Dust into a lasting grave attunement brew.
- **Rite of Infusion: Brew of Soaring**: Combines Phantom Membrane and tagged Flying Ointment into a broom-control brew.
- **Rite of Infusion: Enchanted Broom**: Creates an enchanted broom from a mundane broom and flying ointment.
- **Rite of Infusion: Crystal Ball**: Infuses a quartz sphere for divination.
- **Rite of Infusion: Mirror**: Condenses a craftable silver-and-glass mirror block into a portable infernal mirror.
- **Rite of Infusion: Mystic Branch**: Infuses a branch with focused natural magic.
- **Rite of Infusion: Seer Stone**: Infuses a stone for communication among a coven.
- **Rite of Infusion: Earth**: Infuses those in the circle with the endurance of stone.
- **Rite of Infusion: the Veil**: Infuses those in the circle with otherworldly speed.
- **Rite of Infusion: Infernal**: Infuses those in the circle with protection from flame.
- **Rite of Infusion: Light**: Infuses those in the circle with radiant sight.
- **Rite of Infusion: Sky**: Infuses those in the circle with the grace of flight.
- **Rite of Manifestation**: Temporarily manifests spirit walkers as visible, mobile ghosts.
- **Rite of Handfasting**: Binds two players or a player and an unmarried Nami with golden chalk, two rings, and offerings that grant shared proximity benefits.
- **Rite of Severance**: Ends the caster's current marriage and releases a Nami spouse from her vow.
- **Rite of Nature's Power**: Restores life and strength across a barren area.
- **Rite of Necromancy: Necromantic Stone**: Creates a Necromantic Stone under the night sky.
- **Rite of Broken Earth**: Tears open the earth around the ritual focus.
- **Rite of Prior Incarnation**: Calls restorative echoes of a target player around the circle.
- **Hex of Raining Toads**: Calls poisonous frogs into the ritual area.
- **Rite of Moving Earth**: Raises a solid column from the circle.
- **Rite of Mutation: Storm Simian**: Shapes a flying familiar during a thunderstorm for bound-waystone travel.
- **Rite of Infusion Recharge**: Restores magical reserve to warlocks standing in the circle.
- **Rite of Sanctity**: Protects beings inside the circle from hostile attack.
- **Rite of Necromancy: Spectral Stone**: Creates a Spectral Stone from tagged Attuned Stone, spectral dust, Condensed Fear, and Congealed Spirit under the night sky.
- **Greater Rite of Sky's Wrath**: Calls an extended thunderstorm over the world.
- **Portable Rite of Sky's Wrath**: Calls a focused thunderstorm from a circle talisman.
- **Rite of Sky's Wrath**: Calls a violent thunderstorm over the area.
- **Audience of Naamah**: Summons Naamah, whose dropped blood must be offered to her for vampire initiation.
- **Rite of Summoning: Demon**: Calls a Warlockery demon from the infernal realm.
- **Rite of Summoning: Familiar**: Summons a cat familiar to aid the warlock.
- **Rite of Summoning: Imp**: Calls a flame imp that can be bound through infernal contracts.
- **Rite of Summoning: Banshee**: Calls a banshee from beyond the veil.
- **Rite of Summoning: Poltergeist**: Calls a Poltergeist needed by the greater Rite of Binding.
- **Rite of Summoning: Echo Shade**: Releases the hostile Echo Shade held within an infused mirror.
- **Rite of Summoning: Spectre**: Draws a spectre through the veil into the ritual circle.
- **Rite of Summoning: Witch**: Calls a witch to the center of the prepared circle.
- **Rite of Summoning: Wither**: Calls forth a Wither and requires a clear inner circle.
- **Rite of Calling: Lost Soul**: Calls a Lost Soul that can be bound with a tagged spirit vessel.
- **Rite of Infestation: Parasytic Louse**: Calls a louse that can store and redirect potion effects.
- **Rite of Challenge: Forgewarden**: Calls a combat-ready goblin patron that strengthens nearby goblins.
- **Rite of Challenge: Stonebroker**: Calls an armored goblin patron for combat, worship, and blessings.
- **Rite of the Bloodied Effigy**: Consumes four bloodied wicker bundles at the cardinal points to summon the Thorned Pursuer.
- **Rite of Transposition: Creature**: Summons the creature or player identified by a bound sympathetic imprint.
- **Rite of Transposition: Waystone**: Teleports the caster to a bound waystone in the same dimension.
- **Rite of Transposition: Ore**: Draws ore from the rock below and deposits it at the circle.
- **Rite of Earth's Wrath**: Raises a column of volcanic stone above the circle.

## Doll catalog

- **Doll**: A blank sympathetic vessel used to craft specialized dolls.
- **Earth Guard Doll**: Spends durability to prevent a lethal fall, Elytra collision, dripstone impact, falling-block crush, or mace smash and grants Slow Falling.
- **Water Guard Doll**: Spends durability to prevent lethal drowning, restores the air meter, and grants Water Breathing.
- **Hunger Guard Doll**: Spends durability to prevent lethal starvation, fills the hunger bar, restores saturation, and applies the vanilla Saturation effect.
- **Fire Guard Doll**: Spends durability to prevent lethal fire or lava damage, clears fire, grants Fire Resistance, and searches for safe footing after a lava activation.
- **Tool Mending Doll**: Spends one point of its durability to repair two durability on a damaged held tool.
- **Death Guard Doll**: Spends durability to prevent an otherwise lethal hit and applies the vanilla Totem recovery effects without the Totem screen animation.
- **Hex Guard Doll**: Spends durability to block hostile circle hexes, including Heat Metal, aimed at its bound target.
- **Hexing Doll**: Cycles through prick, shove, ignite, and drown actions against an available bound target.
- **Blood Link Doll**: Spends durability to transfer half of the holder's incoming damage to an available bound target.
- **Doll Guard**: Spends durability to block attempts to corrupt the bound target's protective dolls.
- **Armor Mending Doll**: Spends one point of its durability to repair two durability on damaged worn armor.

The side status panel shows bound dolls, remaining charges, recent activations, and harmful hex or effect durations. Doll activation never uses the full-screen Totem animation.

Right-clicking air binds an eligible protection, mending, or guard doll to the user. A Doll Shelf accepts tagged dolls and sympathetic containers, and a shelf holding a bound doll safely keeps its chunk available so protection and once-per-second equipment mending remain active while its owner travels.

## Item descriptions

### Ritual and sympathetic tools

- **Arcane Focus**: Opens the circle ritual browser and controls the active infusion path on blocks, creatures, or the wielder.
- **Mystic Branch**: Provides direct click-based spell control without gesture recognition.
- **Ritual Chalk**: Draws the standard glyphs used by most circle rites.
- **Infernal Chalk**: Draws infernal glyphs for demon, fire, and Nether rites.
- **Veil Chalk**: Draws glyphs used by spirit, concealment, and otherworld rites.
- **Circle Talisman**: Captures a prepared chalk layout and restores it at a clear destination.
- **Attuned Stone**: Stores portable altar power and transfers charge to or from a nearby altar.
- **Ritual Knife**: Can be placed visibly on an altar as a reusable range focus and removed with shift-right-click.
- **Sympathetic Vial**: Records a creature or player identity for rites, dolls, compasses, and bound devices.
- **Waystone**: Records a location or creature target for travel, divination, and transposition rites.
- **Blooded Waystone**: Carries a creature-linked destination used by advanced sympathetic rites.

### Machines and brewing

- **Altar**: Supplies renewable ritual power when its multiblock and nearby natural environment are valid.
- **Runed Cauldron**: Builds ordered custom brews and reports invalid ingredients, missing heat, fluid, power, or output space.
- **Kettle**: Produces fixed brews from data-driven item and fluid recipes.
- **Alchemical Oven**: Converts tagged materials into ash, jars, and magical fumes.
- **Distillery**: Refines tagged ingredients and fluids into advanced magical reagents.
- **Spinning Wheel**: Converts fibers and magical materials into thread, cloth, and woven components.
- **Brazier**: Burns configured reagents to produce bounded magical effects.
- **Silver Vat**: Collects Silver Deposits whenever an adjacent furnace smelts a common-tagged gold ore, supports several vats and furnaces at once, and exposes the deposits to hoppers and item pipes.

### Travel, divination, and summoning

- **Broom**: Clears tagged chalk glyphs and serves as the base for enchanted flight.
- **Enchanted Broom**: Uses forward thrust with aim-based steering and offers a graceful glide control for descending safely.
- **Flying Ointment**: Supplies the ritual component needed to create and enhance enchanted brooms.
- **Crystal Ball**: Reports weather, moon, progression, and bound-waystone information.
- **Seer Stone**: Provides portable supernatural and progression diagnostics, and calls the owner's loaded recruited Circle Mages when used on a golden circle center.
- **Spectral Stone**: Stores captured spectral creatures for binding and necromancy rites.
- **Necromantic Stone**: Commands tagged undead and supports advanced necromantic interactions.
- **Infernal Mirror**: Holds an Echo Shade and releases it through its summoning rite.
- **Horn of the Hunt**: Rewards completed werewolf progression and summons the Thorned Pursuer.
- **Soul of Torment**: Banishes a non-player target into the generated Abyss dimension.
- **Infernal Animus**: Binds a tagged demon to the wielder's combat commands.

### Combat and protection

- **Silver Bolts**: Fit Minecraft's vanilla crossbow and deal bonus damage to creatures vulnerable to silver.
- **Silver Bolts**: Let players and Werewolf Hunters bypass werewolf protection.
- **Anti-Magic Bolts**: Disrupt magical protections on struck targets.
- **Holy Bolts**: Deal increased damage to tagged demons and undead creatures.
- **Stake Bolts**: Exploit vampire and wooden-weapon vulnerabilities.
- **Goblinite Equipment**: Provides a complete Netherite-tier tool and armor family with fire resistance and common-tag repair materials.
- **Silver Equipment**: Provides a complete repairable tool and armor family whose armor retaliates against attacking werewolves.
- **Werewolf Hunter Equipment**: Protects hunters from supernatural targets and supports silver ranged combat.
- **Ice Slippers**: Use modern Frost Walker behavior to create temporary ice while travelling over water.
- **Sun Grenade**: Releases stored sunlight against vampires and other tagged sunlight-vulnerable creatures.
- **Garlic Garland**: Creates a visible ward that burns vampires attempting to cross it.

### Utility and storage

- **Doll Shelf**: Stores bound dolls while keeping their protection available to the linked player.
- **Rowan Door Key**: Records access to a protected Rowan door.
- **Rowan Keyring**: Combines access for up to sixteen bound Rowan doors.
- **Biting Belt**: Stores and reapplies configured potion effects.
- **Coffin**: Provides the expected sleeping and vampire-rest interactions.
- **Replication Staff**: Copies a bounded safe block selection to a clear destination.
- **Player Compass**: Tracks the player recorded by its sympathetic binding, including across dimensions.
- **Shelf Compass**: Locates the nearest compatible doll shelf.
- **Warlockery Manuals**: Open a searchable manual library with clickable book and chapter navigation.
- **Wedding Rings**: Are offered as a pair during handfasting and receive the names of the newly bound spouses.

## Other ritual-like interactions

- **Vampire initiation**: Bring an unmarried Nami into the Blood Audience during a full moon, witness her become Naamah, defeat Naamah, then offer the dropped Matriarch's Blood to her.
- **Vampire cure**: Bind a sympathetic imprint to the vampire and complete Rite of Hexbreaking: Vampirism.
- **Werewolf capture**: Arm a Silver Werewolf Trap beside a Wolf Altar, keep an adult sheep within eight blocks, and wait for a full-moon night until the trap display shows a green check.
- **Werewolf initiation**: Complete Hex of the Wolf on a bound mortal during its listed moon and coven conditions.
- **Wolf Altar progression**: Complete full-moon Wolf Altar trials with the required head and offering; reaching level ten awards the Horn of the Hunt.
- **Werewolf cure**: Bind a sympathetic imprint to the werewolf and complete Rite of Hexbreaking: Lycanthropy.
- **Marriage**: Stand with another unmarried player or an unmarried Nami inside the prepared golden circle and complete the Rite of Handfasting.
- **Nami marriage**: A married Nami claims one of fifteen world-unique names, follows her spouse, casts harmless-to-terrain combat magic, and returns to the spouse's bed at one health when defeated. Naamah can never marry.
- **Divorce**: Offer a wedding ring, shears, and Foul Fume to the Rite of Severance to clear the only active marriage bond.
- **Hexing link**: Bind a Hexing Doll to a target, sneak-use it to select an action, then use it to apply that action while the target is available.
- **Sympathetic targeting**: Use a Sympathetic Vial on a creature, then place the bound vial where a ritual asks for an imprint.
- **Machine processing**: Insert ingredients into a cauldron or ritual machine and read its floating display for missing inputs, wrong items, heat, fuel, output capacity, and progress.
- **Soul of Torment**: Use the Abyssal Regent's soul on a non-player target to banish it into the generated Abyss dimension.
- **Infernal Animus**: Use the crafted animus on a tagged demon to bind it to the player's combat commands.

## Goblins and hobgoblins

- **Goblins**: Spawn rarely in wooded biomes, hunt ordinary villagers, and leave friendly hobgoblins and their travelling trade role alone.
- **Hobgoblins**: Travel peacefully, receive a miner, smith, shaman, or prospector profession when they appear, and trade or work for players without attacking villagers.
